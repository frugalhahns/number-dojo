...
- **A walkthrough never leaps a step.** If a step carries a nested chain, the
  walkthrough opens it up inline, indented under its parent, and the parent sits
  there with a question mark and the words "too big to do in one go, take it
  apart" until its children have answered it. `432 + 41` is six steps, not
  three, because `32 + 41 = 73` was the one step he could not do and it was
  being presented as a leap. The solve screen keeps this behind a button, since
  there the choice is his, but the button is now the loudest thing in the row.
