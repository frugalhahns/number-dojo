...
import { ri, pick, step, chain, area, groups, numberLine, jump } from '../num.js';
      ...
      return chain({
        ...
        steps: [
          step('Do not divide. Ask a times question instead: how many ' + d + 's make ' + n + '?',
            d + ' × ? = ' + n, q,
            { hint: 'Count up in steps of that size and keep track on your fingers.',
              why: d + ' × ' + q + ' = ' + n + '.' }),
          step('That is it. The missing factor IS the answer.',
            n + ' ÷ ' + d + ' = ?', q,
            { why: 'Dividing by ' + d + ' asks how many ' + d + 's fit. ' + q + ' of them fit.' })
        ],
        board: groups(n, d, 0)
      });
    }
    ...
    return chain({
      ...
      steps: [
        step('Turn it into a times question: how many ' + d + 's make ' + n + '? Start with five of them.',
          d + ' × 5 = ?', easy,
          { hint: 'Half of ' + d + ' × 10, which is ' + (d * 10) + '.', why: 'Five ' + d + 's make ' + easy + '.' }),
        step('You are not at ' + n + ' yet. How much short are you?',
          n + ' − ' + easy + ' = ?', left,
          { hint: 'A small gap.', why: n + ' − ' + easy + ' = ' + left + ' still to cover.' }),
        step('How many more ' + d + 's is that ' + left + '?',
          left + ' ÷ ' + d + ' = ?', more,
          { hint: 'A small one. How many whole groups of that size fit into what is left?', why: left + ' is ' + more + ' more ' + d + '' + (more > 1 ? 's' : '') + '.' }),
        step('Five, plus the extras.', '5 + ' + more + ' = ?', q,
          { why: 'Altogether ' + q + ' ' + d + 's make ' + n + ', so ' + n + ' ÷ ' + d + ' = ' + q + '.' })
      ],
...
export const chunks = {
  ...
  build(p) {
    ...
    const steps = [step('How much is ten groups of ' + d + '?',
      d + ' × 10 = ?', d * 10,
      { hint: 'Times ten just adds a zero.', why: 'Ten ' + d + 's is ' + (d * 10) + '.' })];
    if (bigChunk === 20) {
      steps.push(step('Ten was not enough to reach ' + n + '. Double it: twenty groups.',
        (d * 10) + ' + ' + (d * 10) + ' = ?', taken,
        { hint: 'Just double ' + (d * 10) + '.', why: 'Twenty ' + d + 's is ' + taken + '.' }));
    }
    steps.push(step('Take that chunk out of ' + n + '. What is left to share?',
      n + ' − ' + taken + ' = ?', left,
      { hint: 'Only this bit is left over.', why: n + ' − ' + taken + ' = ' + left + '.' }));
    steps.push(step('Share the leftover ' + left + '.', left + ' ÷ ' + d + ' = ?', more,
      { hint: 'Flip it round: how many whole groups of that size fit in?', why: left + ' ÷ ' + d + ' = ' + more + '.' }));
    steps.push(step('Add your chunks: ' + bigChunk + ' groups and then ' + more + ' more.',
      bigChunk + ' + ' + more + ' = ?', q,
      { why: bigChunk + ' + ' + more + ' = ' + q + ', so ' + n + ' ÷ ' + d + ' = ' + q + '.' }));
    return chain({
      title: n + ' ÷ ' + d, strategy: this.id, answer: q,
  ...
  build(p) {
    ...
    for (let i = 0; i < times; i++) {
      ...
      steps.push(step(i === 0 ? 'Dividing by ' + d + ' means halving ' + times + ' times. Half of ' + v + '?'
                              : 'Halve it again.',
        'half of ' + v + ' = ?', nv,
        { hint: 'Half the tens, half the ones: half of ' + (Math.floor(v / 10) * 10) + ' and half of ' + (v % 10) + '.',
          why: 'Half of ' + v + ' is ' + nv + '.' }));
      v = nv;
    }
    return chain({
    ...
    return chain({
      ...
      steps: [
        step(n + ' is ' + small + ' ' + unit + '. Share the ' + unit + ' instead. Much smaller numbers.',
          small + ' ÷ ' + d + ' = ?', sq,
          { hint: 'Flip it round: how many whole groups of that size fit in?', why: small + ' ÷ ' + d + ' = ' + sq + '.',
            more: sq > 10 ? chunks.build({ n: small, d }) : null }),
        step('Each share is ' + sq + ' ' + unit + '. Say that as an ordinary number.',
          sq + ' ' + unit + ' = ?', q,
          { hint: place === 10 ? 'Stick the zero back on the end.' : 'Stick both zeros back on.',
