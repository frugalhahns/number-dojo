...
import { ri, pick, nextUp, floorTo, step, chain, numberLine, jump, blocks, area } from '../num.js';
    ...
    return chain({
      ...
      steps: [
        step('How many more does ' + a + ' need to reach ' + top + '?',
             a + ' + ? = ' + top, need,
             { hint: 'Only the ones digit matters here. How far is it from there to the next ten?',
               why: a + ' is ' + need + ' short of ' + top + '.' }),
        step('You just spent ' + need + ' of the ' + b + '. How much of it is left?',
             b + ' − ' + need + ' = ?', rest,
             { hint: 'You broke the ' + b + ' into two pieces and spent the first one. Count what is still in your hand.',
               why: b + ' splits into ' + need + ' and ' + rest + '.' }),
        step('Now the easy part. Add what was left to the round ten.',
             top + ' + ' + rest + ' = ?', sum,
             { hint: 'Adding to a round ten never changes the tens digit.',
  ...
  build(p) {
    ...
    if (a >= 100 || b >= 100) {
      ...
      steps.push(step('Hundreds first, and only the hundreds.',
        ah + ' + ' + bh + ' = ?', hun, { hint: 'Think ' + (ah / 100) + ' + ' + (bh / 100) + ', then say it in hundreds.' }));
      steps.push(step('Now just the tens.', at + ' + ' + bt + ' = ?', tens,
        { hint: 'Think ' + (at / 10) + ' + ' + (bt / 10) + ' tens.' }));
      steps.push(step('Now just the ones.', ao + ' + ' + bo + ' = ?', ones, {}));
      steps.push(step('Three piles. Stack them up.', hun + ' + ' + tens + ' + ' + ones + ' = ?', sum,
        { why: 'All the pieces put back together make ' + sum + '.' }));
      running = { hun, tens, ones };
    } else {
      const at = a - ao, bt = b - bo, tens = at + bt;
      steps.push(step('Add the tens, and ignore the ones for now.',
        at + ' + ' + bt + ' = ?', tens,
        { hint: 'That is really just ' + (at / 10) + ' + ' + (bt / 10) + ', in tens.',
          why: at + ' + ' + bt + ' = ' + tens + '.' }));
      steps.push(step('Now the ones, on their own.',
        ao + ' + ' + bo + ' = ?', ones,
        { hint: ones > 10 ? 'This one spills past ten, and that is fine.' : 'A small one.',
          why: ao + ' + ' + bo + ' = ' + ones + '.' }));
      steps.push(step('Put the two piles together.', tens + ' + ' + ones + ' = ?', sum,
        { hint: ones >= 10 ? 'The ones pile is more than ten, so it lifts the tens digit.' : 'Just drop the ones on the end.',
          why: tens + ' + ' + ones + ' = ' + sum + '.' }));
      running = { tens, ones };
    }
    return chain({
    ...
    return chain({
      ...
      steps: [
        step('Nothing is being added down in the ones. What part of ' + a + ' just sits there? (its ' + keptName + ')',
          a + ' + ' + b + ' → keep ?', kept,
          { hint: b + ' has zeros where those digits are, so they cannot change.',
            why: 'The ' + kept + ' is untouched. Put it in your pocket.' }),
        step('Now count in ' + unit + '. ' + a + ' holds ' + big + ' ' + unit + ', and you are adding ' + add + ' more.',
          big + ' + ' + add + ' = ?', core,
          { hint: 'Same little sum you already know. Tap "break it down" if you want help.',
        ...
        step(core + ' ' + unit + ' is ' + (core * place) + '. Now take the ' + kept + ' back out of your pocket.',
          (core * place) + ' + ' + kept + ' = ?', sum,
          { hint: 'It goes right back where it came from.',
    ...
    return chain({
      ...
      steps: [
        step(a + ' is nearly ' + up + '. How much does it have to borrow to get there?',
          a + ' + ? = ' + up, lent,
          { hint: 'Just the ones digit again.', why: a + ' borrows ' + lent + ' to become ' + up + '.' }),
        step('Round tens are easy to add. Do the friendly version.',
          up + ' + ' + b + ' = ?', big,
          { hint: 'The tens digit of ' + up + ' plus the tens of ' + b + ', then the ones of ' + b + '.',
        ...
        step('You borrowed ' + lent + ', so the answer is ' + lent + ' too big. Give it back.',
          big + ' − ' + lent + ' = ?', sum,
          { hint: 'Take away exactly what you borrowed. Not more, not less.',
    ...
    return chain({
      ...
      steps: [
        step('You know your doubles. Double the smaller one.',
          a + ' + ' + a + ' = ?', dbl,
          { hint: 'Doubles are the facts to keep in your pocket.', why: 'Double ' + a + ' is ' + dbl + '.' }),
        step(b + ' is not ' + a + '. How much bigger is it?',
          b + ' − ' + a + ' = ?', off, { hint: 'Only a little.' }),
        step('So the real answer is the double, plus that bit.',
          dbl + ' + ' + off + ' = ?', sum, { why: dbl + ' + ' + off + ' = ' + sum + '.' })
      ],
...
export const ADD = [bridge, split, scale, friendly, near];
